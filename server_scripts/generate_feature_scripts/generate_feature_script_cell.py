...
if retval == 1:
    ...
    filt_att = input_params['filter_attribute']
    filt_att_val = input_params['filter_attribute_value']
    ...
    #-----------------------------------------
    #test_feature_run_all.sh

    #input lines ---
    test_feature_run_all_line1 = 'for f in `seq 1 '+str(1)+'`; do echo "Running feature extraction on server $f"; nohup ssh -i /tmp/id_rsa '+loc_VM+' "'+fs_path_VM+'/test_feature_run_tiles_$f.sh;" & done; \ '

    test_feature_run_all_line2 = 'echo "Running feature extraction on server 0"; nohup '+fs_path_VM+'/test_feature_run_tiles_0.sh; '
    #---
    
    with open("test_feature_run_all.sh","w") as tf:
        tf.write(test_feature_run_all_line1+'\n')
        tf.write(test_feature_run_all_line2+'\n')
    ...
    for i in np.arange(int(num_VM_in)):
        ...
        l5_3 = 'python '+fs_path_VM+'/'+sc_lc+' '+lc_path+' '+clone_data_path+'/'+in_data_dir+'/{}.LAZ '+targ_path+'/{}_target.laz '+str(sr)+' '+clone_data_path+'/'+out_data_dir+'/{}.ply '+filt_att+' '+filt_att_val+';"'
        l5 = l5_1+l5_2+l5_3
    ...
    for i in np.arange(2):
        ...
        #input lines --
        l1 = 'xargs --arg-file=/'+fs_path_VM+'/test_feature_tiles_'+istr+'.sh \\'
        l2 = '      --max-procs='+str(numpr)+'  \\'
        l3 = '      --replace \\'
        l4 = '      --verbose \\'
        l5_1 = '      /bin/sh -c "[ -f '+clone_data_path+'/'+out_data_dir+'/{}.ply ] && echo '
        ...
        with open("test_feature_run_tiles_"+istr+".sh","w") as tf:
            tf.write(l1+'\n')
            tf.write(l2+'\n')
            tf.write(l3+'\n')
            tf.write(l4+'\n')
    ...
    with open("feature_run_tiles_redo.sh","w") as tf:
        tf.write(l1+'\n')
        tf.write(l2+'\n')
        tf.write(l3+'\n')
        tf.write(l4+'\n')
    ...
    l1 = 'echo "Copying feature scripts to server 0"; \\'
    ...
    #l7 = 'echo "creating tmp output directory in cloned data path";  \\'
#   l8 = 'ssh -i ~/.ssh/id_rsa '+loc_master_VM+' "mkdir '+clone_data_path+'/'+out_data_dir+'/ ;" \\'
    l9 = 'for f in `seq 0 '+str(numVM)+'`; do echo "Copying feature scripts to server $f"; echo $f; scp '+sc_lc+' '+loc_VM+':'+fs_path_VM+'/ ; scp feature_run_tiles_$f.sh '+loc_VM+':'+fs_path_VM+'/ ; scp feature_tiles_$f.sh '+loc_VM+':'+fs_path_VM+'/ ; done; '
    ...
    #-----------------------------------------
    #test_feature_copy_files.sh
    
    l1 = 'echo "Copying test feature scripts to server 0"; \\'
    l2 = 'scp test_feature_run_all.sh '+loc_master_VM+':'+fs_path_VM+'/; \\'
    l7 = 'for f in `seq 0 '+str(1)+'`; do echo "Copying test feature scripts to server $f"; echo $f; scp '+sc_lc+' '+loc_VM+':'+fs_path_VM+'/ ; scp test_feature_run_tiles_$f.sh '+loc_VM+':'+fs_path_VM+'/ ; scp test_feature_tiles_$f.sh '+loc_VM+':'+fs_path_VM+'/ ; done; '
    
    with open("test_feature_copy_files.sh", "w") as tf:
        tf.write(l1+'\n')
        tf.write(l7+'\n')
    ...
    #-------------------------------------------
    #feature_move_all.sh
    l1 = 'for f in `seq 1 '+str(numVM)+'`; do echo "Moving feature files on server $f"; nohup ssh -i /tmp/id_rsa '+loc_VM+' "/bin/cp -rf '+clone_data_path+'/'+out_data_dir+'/ '+webdav_path+'/'+out_data_dir+'/ ;" & done; \\'
    l2 = 'echo "Moving feature files on server 0"; nohup /bin/cp -rf '+clone_data_path+'/'+out_data_dir+'/   '+webdav_path+'/'+out_data_dir+'/ ; '
    ...
    l2 = 'nohup /bin/cp -n '+clone_data_path+'/'+out_data_dir+'/ '+webdav_path+'/'+out_data_dir+'/ ; '
